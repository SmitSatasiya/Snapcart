    ...
    <div className="flex flex-col items-center min-h-screen p-6 w-full">
      ...
      <div className="flex flex-col md:flex-row justify-center items-center gap-6 mt-10">
        {roles.map((role) => {
          ...
          const isSelectd = selectedRole == role.id;
          return (
            <motion.div
              key={role.id}
              whileTap={{ scale: 0.94 }}
              onClick={() => setSelectdRole(role.id)}
              className={`flex flex-col items-center justify-center w-48 h-44 rounded-2xl border-2 
                transition-all ${isSelectd ? "border-green-600 bg-green-100 shadow-lg" : "border-gray-300 bg-white hover:border-green-400 cursor-pointer"}`}
            >
              <Icon />
      ...
      >
        <label htmlFor="mobile" className="to-gray-700 font-medium mb-2">
